...
Where $\tau_{\mathrm{swap}}(q)$ dates recovery by counting trees,
$\tau_{\mathrm{err}}(\rho)$ dates it by watching the error itself come back down:
\begin{equation}
  \tau_{\mathrm{err}}(\rho) = \inf\{t > \tau^* : \bar e_t - \hat p_0 \le \rho\,\Delta e\}
  - \tau^* ,
  \label{eq:tauerr}
\end{equation}
the first instant at which the excess error has fallen to a fraction $\rho$ of the jump
it suffered at the drift. We take $\rho = 0.25$ throughout --- three quarters of the
drift absorbed. The value is ours to choose, the assignment fixing only
$\rho \in (0,1)$; what it costs at the weakest amplitudes --- where $\rho\Delta e$ falls
below the noise of $\bar e_t$ and the test decides nothing --- is set out below.
...
$\tau_{\mathrm{rec}}$, is therefore computed on the mean curve $\bar e_t$ alone --- as
$\bar e_t$ in~\eqref{eq:tauerr} already indicates: one value per amplitude, never one
per run.
...
\begin{figure}[htbp]
  ...
  requiring the condition to hold for 20 consecutive steps removes both of the two that
  occur where the test can decide. Hollow markers, inside the shaded band, are the two
  amplitudes where $\rho\Delta e < 2\sigma$: the persistence rule still returns a value
  there, and that value still sits above $\tau_{\mathrm{ARF}}$, but a threshold buried in
  the noise dates the noise rather than the recovery. Those two points are not a verdict
  either way, which is why the count of remaining violations is taken over 18 amplitudes
  and not 20.}
...
\paragraph{Conclusion.} On the 18 amplitudes where the test has power,
$\tau_{\mathrm{ARF}} \le \tau_{\mathrm{err}}(\rho)$ holds without exception: the first
replacement always precedes the actual recovery of the error. That is the optimistic bias
of~\eqref{eq:relation} again, now against a second yardstick.

The conjecture is therefore not confirmed, and at the two weakest amplitudes it is not
refuted either --- the test cannot decide there. Settling them would take more seeds, and
the $1/\sqrt{n}$ law makes that expensive: $194$ at $\Delta e = 0.085$, but $1\,874$ at
$0.028$, and more in practice, a crossing instant being harder to locate where the curve
is flattest.
