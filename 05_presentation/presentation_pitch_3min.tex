...
\begin{frame}{Defence one: a model that repairs itself}
  ...
  \note{The first defence repairs itself. Its building block is a decision
    tree: it splits the picture with simple cuts, always straight across, so
    it can only approximate a slanted boundary by a staircase. One tree is
    fragile, so ten of them vote. That is the Adaptive Random Forest, the
    model we study. And a tree whose answers start going wrong is thrown
    away, while a fresh one grows in its place. Ten trees, replaced one at a
    time.}
\end{frame}
...
\begin{frame}{Defence two: a watchdog that raises the alarm}
  ...
  \note{The second defence comes from a different community. A watchdog
    reads the model's answers one at a time and keeps a pile of the excess
    mistakes. While the model is doing well the pile falls back to zero. Let
    the mistakes come, and it grows, until it crosses a threshold and calls
    a human: audit, retraining, pause. That alarm is the only thing a human
    ever sees. Two communities, two tools, never measured against each
    other.}
\end{frame}
