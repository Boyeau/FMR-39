...
\begin{frame}{Two defences, built by two separate communities}
  ...
  \end{center}
  \note{Two defences against drift, built by two separate communities.
    First, a model that repairs itself: a forest of ten small decision
    trees, and a tree that starts failing is thrown away and replaced.
    Second, a watchdog: it reads the model's mistakes and calls for a human
    once the pile of errors crosses a threshold. That alarm is the only
    thing a human ever sees. Two communities, and nobody had measured their
...
\begin{frame}{Why not just lower the threshold?}
  ...
  \end{center}
  \note{Does the alarm ever fire? That depends on one number: the
    threshold. We re-ran the benchmark at the paper's three settings, two
    thousand runs each. At the highest threshold it fired three times out of
    two thousand. Lower it, and it fires almost every time. So why not lower
    it? Because a low threshold also fires when nothing has happened, and an
    alarm that cries wolf gets switched off. The setting quiet enough to be
    trusted is the one that never sees the drift.}
\end{frame}
...
\begin{frame}{Where we stand}
  ...
  \note{Where we stand. The paradox is real and reproducible, and we can
    ...
    that bias. What remains is to turn this into advice a practitioner can
    use. Thank you.}
\end{frame}
