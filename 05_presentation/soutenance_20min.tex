...
\begin{frame}{Trees share a stream: the paper is optimistic}
  ...
  \note{The second shortcut is independence. The ten trees learn from the same stream, so
    ...
    That is a property of the algorithm, and everything else follows from it. First, the
    sign: the only channel between two trees is the stream they share, and a shared stream
    can only make them fail together, since their covariance reduces to a variance. So the
    real forest repairs more slowly than the independence formula says. Then the size:
    averaging over streams, Jensen's inequality gives a bound, the probability that the
    forest has already repaired itself is at most what the independence formula predicts.
    So the paper makes the forest look faster than it is, and overestimates the blind spot.
    Its critical ensemble size is not wrong, it becomes a conservative ceiling, and the
    paper's own verdict does not change.}
\end{frame}
...
\begin{frame}{The first replacement is the earliest possible date}
  ...
  \note{The paper dates the repair at the first replaced tree. By construction, that is the
    ...
    change from noise. Worse, on a slightly stronger drift it comes later, after three
    hundred and forty-six steps. We checked why with a second control without drift, where we
    only raise the error level by flipping labels. At the same error level, the first
    replacement also waits about two hundred and seventy steps. The internal detector of each
    tree cuts less often when the error is noisier, so on weak drifts this date measures the
...
\begin{frame}{Detecting is not winning}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.53\textwidth}
      ...
      \begin{tabular}{@{}cccc@{}}
        \toprule
        $\lambda$ & alarm & first & false alarms \\
        \midrule
        $8$  & $95.3\%$ & $\mathbf{91.0\%}$ & $4.15\%$ \\
        $10$ & $94.8\%$ & $\mathbf{90.0\%}$ & $1.30\%$ \\
        $12$ & $94.3\%$ & $78.8\%$          & $0.55\%$ \\
    ...
    \begin{column}{0.45\textwidth}
      \centering
      \includegraphics[width=0.95\textwidth]{Fig_QE_compromis_lambda_n2000.png}\\[-0.3em]
      {\scriptsize\color{gray} each point is one $\lambda$; the knee sits just below $\lambda = 12$}
    \end{column}
  \end{columns}
  \begin{itemize}\small
    \item At $\lambda = 15$ the alarm almost always fires, but mostly \emph{after} the repair.
    \item At $\lambda = 10$ the detector still wins $90.0\%$ of races for $1.30\%$ of false
          alarms: the paper's ``no threshold escapes'' does not hold on this bench.
  ...
  \note{One practical consequence. Detecting a drift and winning the race are two different
    ...
    detector wins ninety-one percent of the races, and about four percent of the runs where
    nothing happens raise a false alarm within two thousand steps. At threshold twelve, it
    still wins seventy-nine percent of the races, for half a percent of false alarms. So on
    this bench, low thresholds do escape the blind spot, and the paper's claim that no
    threshold escapes has to be restricted. We did not test the financial streams
