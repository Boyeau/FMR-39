...
\begin{frame}{Trees share a stream: the paper is optimistic}
  ...
  \note{Le second raccourci est l'indépendance. Les dix arbres apprennent sur le même flux,
    ...
    indépendants. Quand on moyenne sur les flux, l'inégalité de Jensen donne un sens clair :
    la probabilité que la forêt se soit déjà réparée est au plus ce que la formule
    d'indépendance prédit. L'article fait donc paraître la forêt plus rapide qu'elle n'est,
    et surestime le point aveugle. Sa taille critique d'ensemble n'est pas fausse, elle
    devient un plafond conservateur, et le verdict de l'article lui-même ne change pas. La
    dépendance pourrait-elle aller dans l'autre sens ? Non : la covariance entre deux arbres
    est égale à une variance, elle n'est donc jamais négative.}
\end{frame}
